...
pub type TcpStreamCompat = StreamWrapper<snowfallio::net::TcpStream>;
#[cfg(unix)]
pub type UnixStreamCompat = StreamWrapper<snowfallio::net::UnixStream>;
...
mod tests {
    ...
    #[snowfallio::test_all]
    async fn test_rw() {
        let listener = snowfallio::net::TcpListener::bind("127.0.0.1:0").unwrap();
        ...
        let client = async {
            let conn = snowfallio::net::TcpStream::connect(addr).await.unwrap();
            let mut compat_conn = TcpStreamCompat::new(conn);
